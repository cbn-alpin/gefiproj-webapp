...
import { GenericTableColor } from '../../globals/generic-table-color';
...
export class GenericTableComponent<T> implements OnInit, AfterViewInit {
  ...
  public get dataObservable(): T[] {
    return this.genericTableData?.map((gd) => gd.data) || [];
  }
  ...
  public isSortDisabled(propertyName: string): boolean {
    try {
      const sortEnabled = this._options.entityTypes.find(
  ...
  public onEdit(event, entity: GenericTableEntity<T>): void {
    ...
    this.historyOfEntitiesUpdating = this.historyOfEntitiesUpdating.concat(
      history
    );
  ...
  public edit(event, entity: GenericTableEntity<T>): void {
    ...
    const genericTableEntityEvent: GenericTableEntityEvent<T> = {
      ...
      callBack: (genericTableEntityErrors?: GenericTableEntityErrors) =>
        this.handleErrors(entity, genericTableEntityErrors),
    };
  ...
  public cancelEditing(event, entity: GenericTableEntity<T>): void {
    event.stopPropagation();
    const previousValue = this.historyOfEntitiesUpdating?.find(
      (history) => history.next === entity
    ).previous;
    entity.data = previousValue.data;
    entity.state = GenericTableEntityState.READ;
    this.historyOfEntitiesUpdating = this.historyOfEntitiesUpdating?.filter(
      (history) => history.next !== entity
    );
  ...
  public cancelCreation(event, entity: GenericTableEntity<T>): void {
    event.stopPropagation();
    this.genericTableData = this.genericTableData?.filter(
  ...
  public isString(entityName: any): boolean {
    return (
      this.options.entityTypes?.find((entity) => entity.code === entityName)
  ...
  public getEntitySelectBoxOptions(entityName: string): SelectBoxOption<any>[] {
    return (
      this.options.entitySelectBoxOptions?.find(
  ...
  public handleErrors(
    ...
    const hasFormErrors = this.handleFormErrors(
      entity,
      genericTableEntityErrors
    );
    if (!hasFormErrors) {
      this.cleanErrors(entity);
      const hasBusinessErrors = this.handleBusinessErrors(
        entity,
        genericTableEntityErrors
      );
      if (!hasBusinessErrors) {
        const hasApiErrors = this.handleApiErrors(
          entity,
          genericTableEntityErrors
        );
        if (!hasApiErrors) {
  ...
  public getPlaceHolder(name: string): string {
    return (
      this.options.entityPlaceHolders?.find(
  ...
  public openDeletionConfirmationDialog(
    event,
    entity: GenericTableEntity<T>
  ): void {
    event.stopPropagation();
  ...
  public isYellowRow(row: GenericTableEntity<T>): boolean {
    return this.getcolor(row) === GenericTableColor.YELLOW ? true : false;
  }

  public isOrangeRow(row: GenericTableEntity<T>): boolean {
    return this.getcolor(row) === GenericTableColor.ORANGE ? true : false;
  }
}
...
@Component({
  ...
  templateUrl:
    '../deletion-confirmation-dialog/deletion-confirmation-dialog.html',
  styleUrls: [
    '../deletion-confirmation-dialog/deletion-confirmation-dialog.scss',
  ],
})
